...
function kopa_custom_front_localization() {
    $front_localization = array(
        'validate' => array(
            'form' => array(
                'submit' => __('Отправлено', kopa_get_domain()),
                'sending' => __('Отправляется...', kopa_get_domain())
            ),
            'name' => array(
                'required' => __('Укажите своё имя', kopa_get_domain()),
                'minlength' => __('At least {0} characters required.', kopa_get_domain())
            ),
            'email' => array(
                'required' => __('Укажите свою почту', kopa_get_domain()),
                'email' => __('Please enter a valid email.', kopa_get_domain())
            ),
            'url' => array(
                'required' => __('Укажите свой сайт', kopa_get_domain()),
                'url' => __('Введите контактную ссылку', kopa_get_domain())
            ),
            'message' => array(
                'required' => __('Введите своё сообщение', kopa_get_domain()),
                'minlength' => __('At least {0} characters required.', kopa_get_domain())
            )
        )
    );
...
function kopa_breadcrumb() {
    if (is_main_query()) {
        ...
        $breadcrumb_home = '<a href="' . home_url() . '">' . __('Главная', kopa_get_domain()) . '</a>';
        $breadcrumb = '';
        ?>
        ...
        if (is_home()) {
            ...
            $description = sprintf(__('<span class="%1$s">Результаты поиска для"%2$s"', kopa_get_domain()), $current_class, $s);
